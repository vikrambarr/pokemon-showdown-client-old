...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'letsgo' | 'metronome' | 'natdex' | 'insurgence' | 'nfe' |
	...
	protected canLearn(speciesid: ID, moveid: ID) {
		...
		if ((this.formatType === 'natdex' || this.formatType === 'insurgence') && move.isNonstandard && move.isNonstandard !== 'Past') {
			return false;
		}
		...
		if (
			...
			(this.dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'insurgence')
		) {
			if (gen === 9) {
				genChar = 'a';
			} else if (gen === 8) {
	...
	getTier(pokemon: Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'insurgence' ? `gen9insurgencepoa` :
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			...
			(dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'insurgence');
		...
		while (learnsetid) {
			...
			if (learnset) {
				for (let moveid in learnset) {
					...
					if (this.formatType !== 'natdex' && this.formatType !== 'insurgence' && move.isNonstandard === "Past") {
						continue;
					}
					if (
		...
		if (sketch || isHackmons) {
			...
			for (let id in BattleMovedex) {
				...
				if (sketch) {
					...
					if (move.isNonstandard === 'Past' && this.formatType !== 'natdex' && this.formatType !== 'insurgence') continue;
					sketchMoves.push(move.id);
				} else {
					if (!(dex.gen < 8 || this.formatType === 'natdex' || this.formatType !== 'insurgence') && move.isZ) continue;
